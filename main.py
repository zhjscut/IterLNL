...
from solver import solver_IterLNL, solver_IterLNL_digit
...
def main(args):
    ...
    cfg.log_dir, logger = collect_logger(prefix=prefix,
                                        ...
                                        #  thr=cfg.thr,
                                        #  noise=1-cfg.percent,
                                         suffix=cfg.suffix if "suffix" in cfg else None
                                         )

    cfg.distributed = get_world_size() > 1
    if cfg.distributed:
    ...
    # for i, samples in enumerate(dataloaders["train_src"].dataset):
    #     logger.info("a sample of train_src")

    # for i, samples in enumerate(dataloaders["train_src"]):
    #     logger.info("a batch of train_src")
    # for i, samples in enumerate(dataloaders["train_tgt"]):
    #     logger.info("a batch of train_tgt")
    # for i, samples in enumerate(dataloaders["train_tgt_no_drop"]):
    #     logger.info("a batch of train_tgt_no_drop")
    # for i, samples in enumerate(dataloaders["test_tgt"]):
    #     logger.info("a batch of test_tgt")
    ...
    if cfg.dataset == "Digit":
        solver = solver_IterLNL_digit(model, dataloaders, evaluator, cfg)
    else:
        solver = solver_IterLNL(model, dataloaders, evaluator, cfg)
    if args.eval_only:
        solver.evaluate()
        return
    # solver.logger.info("before solve...")
    solver.solve()
    return
...
""",
    ...
    parser.add_argument("--amp", action="store_true", default=False,
        help="whether to use auto mixed precision (default: no)")
    ...
    parser.add_argument("--percent", type=float, help="manually set (1 - noise level)")
